...
export async function generateImage(params: { prompt: string; apiKey: string; preferredModel?: string }) {
    ...
    const fortifiedPrompt = `CANDID LIFESTYLE PHOTOGRAPHY, AUTHENTIC MOMENT, FULL LENGTH BODY SHOT. The person MUST be standing ON THE FLOOR and WEARING DETAILED SHOES OR BOOTS. The entire head, hair, and face MUST be fully visible within the frame. ${prompt}. Soft natural lighting, organic skin textures with pores and slight imperfections (NOT AI SMOOTH), visible face clearly showing features, UNPOSED FEEL. Realistic natural hand posture (arms resting at sides or naturally posed). NO MOBILE PHONES, NO CAMERAS, NO MIRRORS, NO FLOATING ARTIFACTS.`;
